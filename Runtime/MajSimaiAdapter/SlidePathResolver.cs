...
internal sealed class SlidePathResolver
{
        ...
        double slideEndTimeSeconds)
    {
        ...
        while (cursor < rawContent.Length)
        {
            ...
            var barCount = StandardSlideBarCounts.Resolve(shape, start, via, end);
            if (barCount <= 0)
        ...
        double slideEndTimeSeconds)
    {
        if (_extendedSlides is null)
            throw new MajSimaiAdaptationException(
                "Extended K Slides require an IExtendedSlideBarCountProvider.");
        ...
        var barCount = _extendedSlides.ResolveBarCount(slideCode);
        if (barCount <= 0)
            throw new MajSimaiAdaptationException(
                $"Non-positive arrow count for extended Slide: {rawContent}");
        return new[]
        {
